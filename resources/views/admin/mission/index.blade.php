        ...
        <div class="wage_content">
            <div class="wage_data">
                <div class="datatable-container">
                    <h2 class="title">قائمة الارسال</h2>
                    <div class="wage_box">
                        <button style="background: #6b8f96; padding:10px 20px; border:none; border-radius:10px; margin:0 0 10px 0;">
                            <a style="color: #ffff" href="{{ route('admin.mission.create') }}">إضافة مهمة جديدة</a>
                        </button>
                    </div>
                    <div class="controls">
                        ...
                        <table id="dataTable" class="datatable">
                            ...
                            <tbody id="table-body">
                                @foreach ($missions as $mission)
                                    <tr>
                                        <td>{{ $mission->id }}</td>
                                        <td>{{ $mission->bank->name }}</td>
                                        <td>{{ $mission->price }}</td>
                                        <td>{{ $mission->reason }}</td>
                                        <td>{{ $mission->created_at->format('Y-m-d') }}</td>
                                        <td>
                                            <a href="{{ route('admin.mission.edit', $mission->id) }}" class="btn btn-warning">تعديل</a>
                                            <form action="{{ route('admin.mission.delete', $mission->id) }}" method="POST" style="display:inline;">
                                                @csrf
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const filterData = () => {
        ...
        Array.from(rows).forEach(row => {
            ...
            const date = new Date(row.children[4].textContent);  // تعديل للعمود الصحيح (التاريخ)
